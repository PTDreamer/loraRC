...
RH_RF22JB* RH_RF22JB::_deviceForInterrupt[RH_RF22_NUM_INTERRUPTS] = {0, 0, 0};
uint8_t RH_RF22JB::_interruptCount = 0; // Index into _deviceForInterrupt for next device
...
PROGMEM static const RH_RF22JB::ModemConfig MODEM_CONFIG_TABLE[] =
{
...
RH_RF22JB::RH_RF22JB(uint8_t slaveSelectPin, uint8_t interruptPin, RHGenericSPI& spi)
    :
...
void RH_RF22JB::setIdleMode(uint8_t idleMode)
{
    _idleMode = idleMode;
}
...
bool RH_RF22JB::init()
{
    ...
    // 8 nibbles(20bit) preamble, 5 nibbles(32bit) preamble detection threshold
    // 2 SYNC words 2d, d4
    // Header length 1 (id)
    ...
    spiWrite(RH_RF22_REG_33_HEADER_CONTROL2, RH_RF22_HDLEN_1 | RH_RF22_SYNCLEN_2);
...
void RH_RF22JB::handleInterrupt()
{
    ...
    if (_lastInterruptFlags[1] & RH_RF22_IPREAVAL)
    {
      ...
      if (_lastInterruptFlags[0] & RH_RF22_IPKVALID)
      {
          ...
          //printBuffer("recv:", _buf + _bufLen, len - _bufLen);
          _rxHeaderId = spiRead(RH_RF22_REG_47_RECEIVED_HEADER3);
        	_rxGood++;
        	_bufLen = len;
        	_mode = RHModeIdle;
        	_rxBufValid = true;
...
uint8_t RH_RF22JB::statusRead()
{
    return spiRead(RH_RF22_REG_02_DEVICE_STATUS);
}

uint8_t RH_RF22JB::adcRead(uint8_t adcsel,
                      uint8_t adcref ,
                      uint8_t adcgain,
                      uint8_t adcoffs)
{
...
uint8_t RH_RF22JB::temperatureRead(uint8_t tsrange, uint8_t tvoffs)
{
...
void RH_RF22JB::setWutPeriod(uint16_t wtm, uint8_t wtr, uint8_t wtd)
{
    uint8_t period[3];
...
// so YMMV
bool RH_RF22JB::setFrequency(float centre, float afcPullInRange)
{
...
// Returns true if centre + (fhch * fhs) is within limits
bool RH_RF22JB::setFHStepSize(uint8_t fhs)
{
...
void RH_RF22JB::setTxPower(uint8_t power)
{
    spiWrite(RH_RF22_REG_6D_TX_POWER, power | RH_RF22_LNA_SW); // On RF23, LNA_SW must be set.
}

// Sets registers from a canned modem configuration structure
void RH_RF22JB::setModemRegisters(const ModemConfig* config)
{
...
bool RH_RF22JB::setModemConfig(ModemConfigChoice index)
{
    if (index > (signed int)(sizeof(MODEM_CONFIG_TABLE) / sizeof(ModemConfig)))
        return false;

    RH_RF22JB::ModemConfig cfg;
    memcpy_P(&cfg, &MODEM_CONFIG_TABLE[index], sizeof(RH_RF22JB::ModemConfig));
    setModemRegisters(&cfg);

    return true;
}

void RH_RF22JB::getModemConfig(ModemConfigChoice index, RH_RF22JB::ModemConfig *cfg)
{
    memcpy_P(cfg, &MODEM_CONFIG_TABLE[index], sizeof(RH_RF22JB::ModemConfig));
}

// REVISIT: top bit is in Header Control 2 0x33
void RH_RF22JB::setPreambleLength(uint8_t nibbles)
{
    spiWrite(RH_RF22_REG_34_PREAMBLE_LENGTH, nibbles);
}

// Caution doesnt set sync word len in Header Control 2 0x33
void RH_RF22JB::setSyncWords(const uint8_t* syncWords, uint8_t len)
{
    spiBurstWrite(RH_RF22_REG_36_SYNC_WORD3, syncWords, len);
}

void RH_RF22JB::clearRxBuf()
{
    ATOMIC_BLOCK_START;
    _bufLen = 0;
    _rxBufValid = false;
...
bool RH_RF22JB::recv(uint8_t* buf, uint8_t* len)
{
    if (!available())
	return false;
...
void RH_RF22JB::restartTransmit()
{
    _mode = RHModeIdle;
    _txBufSentIndex = 0;
...
void RH_RF22JB::setRadioID(int val) {
    _txHeaderId = val;
    spiWrite(RH_RF22_REG_3A_TRANSMIT_HEADER3, _txHeaderId);
}

bool RH_RF22JB::send(const uint8_t* data, uint8_t len)
{
    bool ret = true;
    waitPacketSent();
...
bool RH_RF22JB::fillTxBuf(const uint8_t* data, uint8_t len)
{
    clearTxBuf();
    if (!len)
	return false;
...
bool RH_RF22JB::appendTxBuf(const uint8_t* data, uint8_t len)
{
...
void RH_RF22JB::sendNextFragment()
{
...
void RH_RF22JB::readNextFragment()
{
...
void RH_RF22JB::handleExternalInterrupt()
{
}

// Default implmentation does nothing. Override if you wish
void RH_RF22JB::handleWakeupTimerInterrupt()
{
}

void RH_RF22JB::setPromiscuous(bool promiscuous)
{
    RHSPIDriver::setPromiscuous(promiscuous);
    spiWrite(RH_RF22_REG_43_HEADER_ENABLE3, promiscuous ? 0x00 : 0xff);
}

bool RH_RF22JB::setCRCPolynomial(CRCPolynomial polynomial)
{
...
uint8_t RH_RF22JB::maxMessageLength()
{
    return RH_RF22_MAX_MESSAGE_LEN;
}
//NOT USED JB
void RH_RF22JB::setThisAddress(uint8_t thisAddress)
{
    return;
}

uint32_t RH_RF22JB::getLastPreambleTime()
{
    return _lastPreambleTime;
}

void RH_RF22JB::setGpioReversed(bool gpioReversed)
{
